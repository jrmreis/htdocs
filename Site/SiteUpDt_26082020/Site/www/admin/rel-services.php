...
if(isset($_GET['excluir'])){
	...
	$result = $mga->update($table,array('deleted'),array('1'),'id = "'.$excluir.'"');
	if(isset($_GET['ft'])){
...
if(isset($_GET['desativar'])){
	$result = $mga->update($table,array('active'), array('0'), 'id = "'.$_GET['desativar'].'"');
	header("Location: ".$url."?success=1");
	exit();
}

if(isset($_GET['ativar'])){
	$result = $mga->update($table,array('active'), array('1'), 'id = "'.$_GET['ativar'].'"');
	header("Location: ".$url."?success=1");
	exit();
}
?>
                            ...
                            <tbody>
                            	<?php 
                            		...
                            		while($qryr=mysql_fetch_array($sqlr)){
	                            		$fold = str_replace('{$lastid}', $qryr['id'], $folder);
	                            		$arquivo = is_file($fold.$qryr['image']) ? '<a href="'.$fold.$qryr['image'].'" class="lightbox">Visualizar Imagem</a>' : '';
                            	?>
                                <tr>
			                        <td align="center"><?=$qryr['id']?></td>
			                        <td><?=$qryr['title']?></td>
			                        <td><?=$arquivo?></td>
			                        <td><?=$VARS['ativo'][$qryr['active']]?></td>
			                        <td>

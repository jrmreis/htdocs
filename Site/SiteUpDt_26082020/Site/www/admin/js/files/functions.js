$(function() {
	...
	$("#wizard3").formwizard({ 
		...
		inDuration : 400,
		outDuration: 400,
		easing: 'easeInBack'	//see e.g. https://gsgd.co.uk/sandbox/jquery/easing/ for information on easing
	 }
	);
	...
	$(".cs-alert").click(function(e) {
		e.preventDefault();
		bootbox.alert($(this).attr('data-mensagem'));
	});
	...
	$("a.generic-modal").click(function(e) {
		e.preventDefault();
		bootbox.modal('<img src="https://dummyimage.com/600x400/000/fff" alt=""/>', 'Modal popup!');
	});
	...
	$('.number_format').maskMoney({prefix:'', allowNegative: false, thousands:'', decimal:',', affixesStay: false});
	$('.peso_format').number(true, 3, '.', '');
	
	$('.colorpicker').colorpicker({
		format: 'hex'
	});

	$(".js-categorie").change(function(){
		$(".js-subcategorie").html('<option>Carregando Subcategorias...</option>');
		ajaxJSON('e', $(this).val(), ".js-subcategorie");
	});

	oTable = $(".rel-table").dataTable({
		"bJQueryUI": false,
		"bAutoWidth": true,
		"sPaginationType": "full_numbers",
		"sDom": '<"datatable-header"fl>t<"datatable-footer"ip>',

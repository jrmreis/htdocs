				...
				<div class="widget">
                    ...
                    <div class="table-overflow">
                        <table class="table table-striped table-bordered table-checks rel-table">
                            ...
                            <tbody>
                            	<?php 
                            		$sqlr = $mga->getRows($table.' as sc',
                            		'sc.*, s.title as service',
                            		'inner join up_services as s on(s.id = sc.service_id)',
                            		'',
                            		'')or die(mysql_error());
                            		while($qryr=mysql_fetch_array($sqlr)){
	                            		// $fold = str_replace('{$lastid}', $qryr['id'], $folder);
	                            		// $arquivo = is_file($fold.$qryr['imagem']) ? '<a href="'.$fold.$qryr['imagem'].'" class="lightbox">Visualizar Imagem</a>' : '';
                            	?>
                                <tr>
